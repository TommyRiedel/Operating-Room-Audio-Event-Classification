...
def spec_augment(mel_spectrogram, time_warping_para=80, frequency_masking_para=27,
    ...
    mel_spectrogram_holder = tf.compat.v1.placeholder(tf.float32, shape=[1, v, tau, 1])
    location_holder = tf.compat.v1.placeholder(tf.float32, shape=[1, 1, 2])
    destination_holder = tf.compat.v1.placeholder(tf.float32, shape=[1, 1, 2])
