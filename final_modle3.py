...
for item in data_dir_train:
    ...
    for room in all_classes:
        class_labels_train.append((item, str('/Users/thomasriedel/spectrograms/Training/' +
                                             '/' + item) + '/' + room))
...
for item in data_dir_test:
    ...
    for room in all_classes:
        class_labels_test.append((item, str('/Users/thomasriedel/spectrograms/Testing/'
                                            + '/' + item) + '/' + room))
...
for item in data_dir_vali:
    all_classes = os.listdir('/Users/thomasriedel/spectrograms/Validation/' + '/' + item)
    if '.DS_Store' in all_classes:
        all_classes.remove('.DS_Store')

    for room in all_classes:
        class_labels_vali.append((item, str('/Users/thomasriedel/spectrograms/Validation/'
                                            + '/' + item) + '/' + room))
...
for item in data_dir_aug:
    all_classes = os.listdir('/Users/thomasriedel/spectrograms_final/AugmentationTest/'
                             + '/' + item)
    if '.DS_Store' in all_classes:
        all_classes.remove('.DS_Store')

    for room in all_classes:
        class_labels_aug.append((item, str('/Users/thomasriedel/spectrograms_final/AugmentationTest/'
                                           + '/' + item) + '/' + room))
...
for i in data_dir_aug:
    data_path = os.path.join('/Users/thomasriedel/spectrograms_final/AugmentationTest/', str(i))
    filenames = [i for i in os.listdir(data_path)]
    if '.DS_Store' in filenames:
        filenames.remove('.DS_Store')
    for f in filenames:
...
def build_model(num_classes):
    ...
    outputs = layers.Dense(NUM_CLASSES, kernel_regularizer=regularizers.l2(0.01),
                           activation="softmax", name="pred")(x)
...
hist = model.fit(np.array(images_train), Y_train, batch_size = 256, epochs=100, verbose=2,
...
class_names = ['mono Coag.','bi Coag.','Cutting','Hämolock','Table up','Table down',
               'tilt table','Table forth','Phone','DaVinci','Idle']
...
sns.heatmap(cmn, annot=True, fmt='.2f', xticklabels=class_names,
            yticklabels=class_names, cmap=plt.cm.Blues)
...
class_names_aug = ['mono Coag.','bi Coag.','Hämolock','Table up','Table down',
                   'tilt table','Table forth','DaVinci','Idle']
...
fig, ax = plt.subplots(figsize=(11,  11))
sns.heatmap(cmn_aug, annot=True, fmt='.2f', xticklabels=class_names_aug,
            yticklabels=class_names_aug, cmap=plt.cm.Blues)
plt.xticks(rotation=45)
plt.yticks(rotation=45)
